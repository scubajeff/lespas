...
class MainActivity : AppCompatActivity() {
                ...
                if (!sp.getBoolean(
                    ...
                    intent.getStringExtra(LesPasArtProvider.FROM_MUZEI_ALBUM)?.let {
                        ...
                        lifecycleScope.launch {
                            ...
                            else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) registerForActivityResult(
                                ActivityResultContracts.RequestPermission()
                            ) {}.launch(android.Manifest.permission.ACCESS_MEDIA_LOCATION)

                            // If Snapseed is not installed, disable Snapseed integration
                            packageManager.getLaunchIntentForPackage(SettingsFragment.SNAPSEED_PACKAGE_NAME)
                                ?: run {
                                    sp.edit {
                                        putBoolean(
                            ...
                            ContentResolver.setSyncAutomatically(
                                accounts[0], getString(R.string.sync_authority), true
                            )
                        }

                        when (intent.action) {
                            LAUNCH_GALLERY -> supportFragmentManager.beginTransaction().add(
                                R.id.container_root,
                                GalleryFragment(),
                                GalleryFragment.TAG_FROM_LAUNCHER
                            ).commit()

                            Intent.ACTION_VIEW -> intent.data?.let {
                                supportFragmentManager.beginTransaction().add(
                                    R.id.container_root,
                                    GalleryFragment.newInstance(it),
                                    GalleryFragment.TAG_FROM_LAUNCHER
                                ).commit()
                            }

                            Intent.ACTION_PICK -> supportFragmentManager.beginTransaction().add(
                                R.id.container_root,
                                GalleryFragment.newInstance(Uri.EMPTY),
                                GalleryFragment.TAG_FROM_LAUNCHER
                            ).commit()

                            else -> supportFragmentManager.beginTransaction().add(
                                R.id.container_root,
                                if (isTV) TVMainFragment() else AlbumFragment.newInstance()
                            ).commit()
                        }
                    }
                }
                ...
                window.decorView.post {
                    lifecycleScope.launch(Dispatchers.IO) {
                        StatFs(Environment.getDataDirectory().path).let {
                            if (it.availableBlocksLong < it.blockCountLong / 10) withContext(
                                Dispatchers.Main
                            ) {
                                if (supportFragmentManager.findFragmentByTag(
                                        CONFIRM_LOW_STORAGE_SPACE_DIALOG
                                    ) == null
                                ) ConfirmDialogFragment.newInstance(
                                    getString(R.string.msg_low_storage_space),
                                    cancelable = false,
                                    requestKey = MAIN_ACTIVITY_REQUEST_KEY
                                ).show(supportFragmentManager, CONFIRM_LOW_STORAGE_SPACE_DIALOG)
                            }
                        }
                    }
                }
    ...
    override fun onStart() {
        ...
        if (accounts.isNotEmpty()) sp.registerOnSharedPreferenceChangeListener(
    ...
    override fun onResume() {
        ...
        if (accounts.isNotEmpty() && AccountManager.get(this)
                .getAccountsByType(getString(R.string.account_type_nc)).isEmpty()
        ) finishAndRemoveTask()
    }
    ...
    override fun onStop() {
        ...
        if (prefBackupNeeded) {
            actionsPendingModel.addAction(
                Action(
                    null,
                    Action.ACTION_BACKUP_PREFERENCE,
                    "",
                    "",
                    "",
                    "",
        ...
        if (accounts.isNotEmpty()) sp.unregisterOnSharedPreferenceChangeListener(
            backupPreferenceListener
        )
    ...
    fun observeTransferWorker() {
        WorkManager.getInstance(this)
            .getWorkInfosForUniqueWorkLiveData(TransferStorageWorker.WORKER_NAME)
            .observe(this) { workInfos ->
                try {
                    workInfos?.get(0)?.apply {
                        if (state.isFinished) {
                            if (supportFragmentManager.findFragmentByTag(CONFIRM_RESTART_DIALOG) == null) ConfirmDialogFragment.newInstance(
                                getString(R.string.need_to_restart),
                                cancelable = false,
                                individualKey = CONFIRM_RESTART_DIALOG,
                                requestKey = MAIN_ACTIVITY_REQUEST_KEY
    ...
    private val backupPreferenceListener =
        SharedPreferences.OnSharedPreferenceChangeListener { _, key ->
            when (key) {
                ...
                getString(R.string.sync_deletion_perf_key), getString(R.string.backup_status_pref_key), getString(
                    R.string.sync_status_local_action_pref_key
                ), getString(R.string.sync_status_pref_key), SyncAdapter.LATEST_ARCHIVE_FOLDER_ETAG -> {
                }
