...
class ShareReceiverActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if ((intent.action == Intent.ACTION_SEND) && (intent.type?.startsWith("image/")!! || intent.type?.startsWith("video/")!!)) files.add(intent.parcelable<Parcelable>(Intent.EXTRA_STREAM) as Uri)
        ...
        if (files.isNotEmpty()) {
            ...
            if (supportFragmentManager.findFragmentByTag(TAG_DESTINATION_DIALOG) == null) {
                DestinationDialogFragment.newInstance(
                    ...
                    files,
                    intent.flags and Intent.FLAG_GRANT_WRITE_URI_PERMISSION > 0 || intent.getBooleanExtra(KEY_SHOW_REMOVE_OPTION, false),
