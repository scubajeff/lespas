...
class SettingsFragment : PreferenceFragmentCompat(),
    ...
    private val authenticateModel: NCLoginFragment.AuthenticateViewModel by activityViewModels {
        NCLoginFragment.AuthenticateViewModelFactory(
            requireActivity()
        )
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        snapseedPermissionRequestLauncher =
            registerForActivityResult(ActivityResultContracts.RequestMultiplePermissions()) { results ->
                ...
                findPreference<SwitchPreferenceCompat>(getString(R.string.snapseed_pref_key))?.isChecked =
                    isGranted

                if (isGranted) {
                    // Explicitly request ACCESS_MEDIA_LOCATION permission
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) accessMediaLocationPermissionRequestLauncher.launch(
        ...
        showGalleryPermissionRequestLauncher =
            registerForActivityResult(ActivityResultContracts.RequestMultiplePermissions()) { results ->
                requireActivity().requestedOrientation = ActivityInfo.SCREEN_ORIENTATION_UNSPECIFIED

                var isGranted = true
                for (result in results) isGranted = isGranted && result.value
                findPreference<SwitchPreferenceCompat>(getString(R.string.gallery_as_album_perf_key))?.isChecked =
                    isGranted

                // Explicitly request ACCESS_MEDIA_LOCATION permission
                if (isGranted && Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) accessMediaLocationPermissionRequestLauncher.launch(
                    android.Manifest.permission.ACCESS_MEDIA_LOCATION
                )
            }/*
        ...
        */
        installSnapseedLauncher =
            registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
                isSnapseedNotInstalled =
                    requireContext().packageManager.getLaunchIntentForPackage(SNAPSEED_PACKAGE_NAME) == null
                if (!isSnapseedNotInstalled) findPreference<SwitchPreferenceCompat>(getString(R.string.snapseed_pref_key))?.isChecked =
                    true
            }

        manageMediaPermissionRequestLauncher =
            registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {}
        ...
        requireActivity().addMenuProvider(object : MenuProvider {
            ...
            override fun onPrepareMenu(menu: Menu) {
                ...
                if (tag == GalleryOverviewFragment.LAUNCH_BY_GALLERY) {
                    archiveMenuItem =
                        menu.findItem(R.id.option_menu_archive)?.apply { isVisible = false }
                    refreshArchiveMenuItem = menu.findItem(R.id.option_menu_archive_forced_refresh)
    ...
    override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
        volume =
            (requireContext().getSystemService(Context.STORAGE_SERVICE) as StorageManager).storageVolumes
        accounts = AccountManager.get(requireContext())
            .getAccountsByType(getString(R.string.account_type_nc))
        ...
        findPreference<SwitchPreferenceCompat>(getString(R.string.true_black_pref_key))?.run {
            if (sharedPreferences?.getString(
                    getString(R.string.auto_theme_perf_key), getString(R.string.theme_auto_values)
                ) == getString(R.string.theme_light_values)
            ) {
        ...
        findPreference<Preference>(getString(R.string.transfer_pref_key))?.let {
            ...
            if (it.isVisible) {
                ...
                it.title =
                    getString(if (inInternal) R.string.transfer_to_external else R.string.transfer_to_internal)
                ...
                lifecycleScope.launch(Dispatchers.IO) {
                    ...
                    withContext(Dispatchers.Main) {
                        if (inInternal && volume[1].state != Environment.MEDIA_MOUNTED) {
                            ...
                            if (!it.isEnabled) it.summary = getString(
                                R.string.not_enough_space_message,
                                Tools.humanReadableByteCountSI(totalSize),
                                getString(if (inInternal) R.string.external_storage else R.string.internal_storage)
                            )
                        }
                    }
                }
//                }
        ...
        */
        ...
        findPreference<Preference>(getString(R.string.cache_size_pref_key))?.run {
            summary =
                getString(R.string.cache_size_summary, sharedPreferences!!.getInt(CACHE_SIZE, 800))
        }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        parentFragmentManager.setFragmentResultListener(
            SETTING_REQUEST_KEY, viewLifecycleOwner
        ) { _, bundle ->
            if (bundle.getBoolean(ConfirmDialogFragment.CONFIRM_DIALOG_RESULT_KEY, false)) {
                when (bundle.getString(ConfirmDialogFragment.INDIVIDUAL_REQUEST_KEY, "")) {
                    LOGOUT_CONFIRM_DIALOG -> {
                        AccountManager.get(context)
                            .apply { removeAccountExplicitly(getAccountsByType(getString(R.string.account_type_nc))[0]) }
                    ...
                    INSTALL_SNAPSEED_DIALOG -> try {
                        installSnapseedLauncher.launch(
                            Intent(
                                Intent.ACTION_VIEW,
                                "market://details?id=${SNAPSEED_PACKAGE_NAME}".toUri()
                            )
                        )
                        ...
                        installSnapseedLauncher.launch(
                            Intent(
                                Intent.ACTION_VIEW,
                                "https://play.google.com/store/apps/details?id=${SNAPSEED_PACKAGE_NAME}".toUri()
                            )
                        )
                    }
                    ...
                    */
                    MANAGE_MEDIA_PERMISSION_RATIONALE_REQUEST -> if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) manageMediaPermissionRequestLauncher.launch(
                        Intent(
                ...
                when (bundle.getString(ConfirmDialogFragment.INDIVIDUAL_REQUEST_KEY, "")) {
                    INSTALL_SNAPSEED_DIALOG, SNAPSEED_PERMISSION_RATIONALE_REQUEST_DIALOG -> findPreference<SwitchPreferenceCompat>(
        ...
        findPreference<PreferenceCategory>(getString(R.string.blog_category_pref_key))?.isVisible =
            ViewModelProvider(requireActivity())[NCShareViewModel::class.java].isBlogSiteAvailable()
    ...
    override fun onResume() {
        ...
        storageStatisticSummaryString?.let {
            findPreference<Preference>(getString(R.string.storage_statistic_pref_key))?.summary = it
        }
    ...
    override fun onDestroyView() {
        if (syncWhenClosing) {
            ContentResolver.requestSync(
                accounts[0], getString(R.string.sync_authority), Bundle().apply {
    ...
    override fun onPreferenceTreeClick(preference: Preference): Boolean = when (preference.key) {/*
        ...
        getString(R.string.logout_pref_key) -> {
            if (parentFragmentManager.findFragmentByTag(CONFIRM_DIALOG) == null) ConfirmDialogFragment.newInstance(
                getString(R.string.msg_logout_dialog, accounts[0].name),
                positiveButtonText = getString(R.string.yes_logout),
                individualKey = LOGOUT_CONFIRM_DIALOG,
        ...
        getString(R.string.transfer_pref_key) -> {
            if (ContentResolver.isSyncActive(accounts[0], getString(R.string.sync_authority))) {
                if (parentFragmentManager.findFragmentByTag(CONFIRM_DIALOG) == null) ConfirmDialogFragment.newInstance(
                    getString(R.string.is_syncing_message),
                    cancelable = false,
                    requestKey = SETTING_REQUEST_KEY
                ).show(parentFragmentManager, CONFIRM_DIALOG)
            } else if (parentFragmentManager.findFragmentByTag(TRANSFER_FILES_DIALOG) == null) TransferStorageDialog.newInstance(
                getString(R.string.confirm_transferring_message, preference.title)
            ).show(parentFragmentManager, TRANSFER_FILES_DIALOG)
            true
        }
        ...
        getString(R.string.relogin_pref_key) -> {
            ...
            AccountManager.get(requireContext()).run {
                ...
                val loginName =
                    getUserData(account, getString(R.string.nc_userdata_loginname)) ?: getUserData(
                        account, getString(R.string.nc_userdata_username)
                    )
                authenticateModel.run {
                    ...
                    setSelfSigned(
                        getUserData(
                            account, getString(R.string.nc_userdata_selfsigned)
                        ).toBoolean()
                    )
                    try {
                        getUserData(
                            account, getString(R.string.nc_userdata_certificate)
            ...
            parentFragmentManager.beginTransaction().replace(
                R.id.container_root,
                NCAuthenticationFragment.newInstance(true),
                NCAuthenticationFragment::class.java.canonicalName
                ...
                if (shouldShowRequestPermissionRationale(if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) android.Manifest.permission.READ_MEDIA_IMAGES else android.Manifest.permission.READ_EXTERNAL_STORAGE)) {
                    if (parentFragmentManager.findFragmentByTag(CONFIRM_DIALOG) == null) {
                        ConfirmDialogFragment.newInstance(
                            getString(R.string.storage_access_permission_rationale),
                            positiveButtonText = getString(R.string.proceed_request),
                            individualKey = SNAPSEED_PERMISSION_RATIONALE_REQUEST_DIALOG,
                            requestKey = SETTING_REQUEST_KEY
                        ).show(parentFragmentManager, CONFIRM_DIALOG)
                    }
                } else {
        ...
        getString(R.string.clear_cache_pref_key) -> {
            if (parentFragmentManager.findFragmentByTag(CONFIRM_DIALOG) == null) ConfirmDialogFragment.newInstance(
                getString(R.string.msg_clear_cache),
                individualKey = CLEAR_CACHE_CONFIRM_DIALOG,
                requestKey = SETTING_REQUEST_KEY
            ).show(parentFragmentManager, CONFIRM_DIALOG)
            true
        }
        ...
        getString(R.string.cache_size_pref_key) -> {
            if (parentFragmentManager.findFragmentByTag(CACHE_SIZE_DIALOG) == null) CacheSizeSettingDialog().show(
                parentFragmentManager, CACHE_SIZE_DIALOG
            )
            true
        }
        ...
        getString(R.string.sync_status_pref_key) -> {
            if (parentFragmentManager.findFragmentByTag(SYNC_STATUS_DIALOG) == null) SyncStatusDialogFragment().show(
                parentFragmentManager, SYNC_STATUS_DIALOG
            )
            true
        }
    ...
    override fun onSharedPreferenceChanged(sharedPreferences: SharedPreferences?, key: String?) {
        when (key) {
            ...
            CACHE_SIZE -> sharedPreferences?.let {
                findPreference<Preference>(getString(R.string.cache_size_pref_key))?.summary =
                    getString(R.string.cache_size_summary, it.getInt(CACHE_SIZE, 800))
            }
            ...
            getString(R.string.sync_deletion_perf_key) -> if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S && !MediaStore.canManageMedia(
                    requireContext()
                )
    ...
    */
    ...
    private fun installSnapseedIfNeeded() {
        if (preferenceManager.sharedPreferences?.getBoolean(
                getString(R.string.snapseed_pref_key), false
            ) == true && isSnapseedNotInstalled
        ) {
            // Prompt user to install Snapseed
            if (parentFragmentManager.findFragmentByTag(CONFIRM_DIALOG) == null) ConfirmDialogFragment.newInstance(
                getString(R.string.install_snapseed_dialog_msg),
                individualKey = INSTALL_SNAPSEED_DIALOG,
                requestKey = SETTING_REQUEST_KEY
            ).show(parentFragmentManager, CONFIRM_DIALOG)
        }
    }
    ...
    */
    ...
    private fun showStatistic(preference: Preference) {
        viewLifecycleOwner.lifecycleScope.launch(Dispatchers.IO) {
            // Database statistic
            storageStatisticSummaryString = getString(
                R.string.statistic_db_message,
                PhotoRepository(requireActivity().application).getPhotoTotal(),
                AlbumRepository(requireActivity().application).getAlbumTotal()
            )
            ...
            storageStatisticSummaryString = storageStatisticSummaryString + "\n" + getString(
                R.string.statistic_storage_message, Tools.humanReadableByteCountSI(
                    if (totalSize == -1L) Tools.getStorageSize(
                        requireContext()
                    ) else totalSize
                ), getString(
                    if (PreferenceManager.getDefaultSharedPreferences(requireContext())
                            .getBoolean(KEY_STORAGE_LOCATION, true)
                    ) R.string.internal_storage else R.string.external_storage
                )
            )
            withContext(Dispatchers.Main) { preference.summary = storageStatisticSummaryString }
            storageStatisticSummaryString = storageStatisticSummaryString + "\n" + getString(
                R.string.statistic_free_space_message,
                Tools.humanReadableByteCountSI(requireContext().filesDir.freeSpace),
                getString(R.string.internal_storage)
            )

            if (volume.size > 1 && volume[1].state == Environment.MEDIA_MOUNTED) try {
                storageStatisticSummaryString = storageStatisticSummaryString + "\n" + getString(
                    R.string.statistic_free_space_message, Tools.humanReadableByteCountSI(
    ...
    private fun isEnoughSpace(sp: SharedPreferences): Boolean =
        // Add 100MB redundant
        try {
            (if (sp.getBoolean(KEY_STORAGE_LOCATION, true)) requireContext().getExternalFilesDirs(
                null
            )[1] else requireContext().filesDir).freeSpace > totalSize + 100 * 1024 * 1024
    ...
    class TransferStorageDialog : LesPasDialogFragment(R.layout.fragment_transfer_storage_dialog) {
        override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
            ...
            view.findViewById<TextView>(R.id.dialog_title)
                .apply { text = arguments?.getString(MESSAGE) }
            view.findViewById<MaterialButton>(R.id.ok_button).setOnClickListener {
                WorkManager.getInstance(requireContext()).enqueueUniqueWork(
                    TransferStorageWorker.WORKER_NAME,
                    ExistingWorkPolicy.KEEP,
                    OneTimeWorkRequestBuilder<TransferStorageWorker>().build()
                )
        ...
        companion object {
            ...
            @JvmStatic
            fun newInstance(message: String) = TransferStorageDialog().apply {
    ...
    companion object {
        ...
        private const val SNAPSEED_PERMISSION_RATIONALE_REQUEST_DIALOG =
            "SNAPSEED_PERMISSION_RATIONALE_REQUEST_DIALOG"
        ...
        */
        ...
        private const val MANAGE_MEDIA_PERMISSION_RATIONALE_REQUEST =
        ...
        */
        ...
        const val SNAPSEED_MAIN_ACTIVITY_CLASS_NAME =
            "com.google.android.apps.snapseed.MainActivity"
