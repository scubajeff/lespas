        ...
        try {
            ...
            requireArguments().getParcelable<Photo>(KEY_MEDIA)?.apply {
                ...
                with("${Tools.getLocalRoot(requireContext())}/${if (eTag.isNotEmpty()) id else name}") {
                    size = Tools.humanReadableByteCountSI(File(this).length())
                ...
                requireArguments().getParcelable<NCShareViewModel.RemotePhoto>(KEY_REMOTE_MEDIA)?.also { media->
                    ...
                    exif = ViewModelProvider(requireActivity()).get(NCShareViewModel::class.java).getMediaExif(media)
                }
            }

            view.findViewById<TextView>(R.id.info_filename).text = name
            view.findViewById<TextView>(R.id.info_shotat).text = dateString
            view.findViewById<TextView>(R.id.info_size).text = if (size.isEmpty()) String.format("%sw × %sh", widthString, heightString) else String.format("%s, %s", size, String.format("%sw × %sh", widthString, heightString))
